...
async function createTasksFromTemplate(tpl, occurrenceDateUtc, { department_id = null, preview = false, force = false } = {}) {
  ...
  } else if (tpl.assignment_mode === 'specific_users') {
    const ids = Array.isArray(tpl.specific_user_ids) ? tpl.specific_user_ids : [];
    if (ids.length > 0) {
      const q = { _id: { $in: ids }, is_active: true };
      // if a department scope is set on template, enforce it for tasks
      if (deptId) q.department_id = deptId;
      users = await User.find(q).select('_id department_id');
    } else {
      users = [];
    }
  }
