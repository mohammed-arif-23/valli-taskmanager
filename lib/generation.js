...
function getIstMidnightUtc(date) {
  // Convert input date to IST, then construct midnight, then convert to UTC
...
function dueAtUtcFromIstDay(istDayUtcMidnight, hhmm) {
  ...
  // Convert IST midnight UTC -> actual IST date, add time, back to UTC
...
async function createTasksFromTemplate(tpl, occurrenceDateUtc, { department_id = null, preview = false, force = false } = {}) {
  ...
  // Determine items: if none present, treat template as a single implicit item for backward compatibility
  ...
  // Helper to upsert or create a task (or count if preview)
  async function createOrUpdateTask(payload, dedupeQuery) {
    if (preview) {
      // If exists and not forcing, don't count; if force, count as update
    ...
    if (existing) {
      if (force) {
        // Update due time and points and description fields from item/template
        ...
        await existing.save();
        return 0; // treat updates not as new creations in count
      }
      return 0;
    }
...
/**
 * Generate daily tasks for a given IST calendar day.
 * Idempotent: checks existing tasks by (template, occurrence_date, department/user) to avoid duplicates.
 */
...
/**
 * Generate recurring tasks (daily/weekly/monthly) for a given IST calendar day.
 * Weekly: runs when today's IST weekday is in template.days_of_week.
 * Monthly: runs on 1st of the month (IST) by default.
 */
